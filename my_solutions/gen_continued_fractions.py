...
# This code is taken from Dr. David Biersach
...
def decode_gencf(form: tuple[int, ...]) -> float:
    ...
    # loop to decode the continued fraction
...
def main() -> None:
    ...
    eulerApprox: float = decode_gencf((3, 1, 4, 4, 1, 0, 6))
    print_rel_error(eulerApprox, math.pi)

    # Calculate the digits of Pi using Biersach's first GCF
    print("Biersach's Generalized Continued Fraction for Pi (v1)")
    drBApprox1 = decode_gencf((3, 1, 8, 0, -7, 8, -1))
    print_rel_error(drBApprox1, math.pi)

    # Calculate the digits of Pi using Biersach's second GCF
    print("Biersachs's Generalized Continued Fraction for Pi (v2)")
    drBApprox2 = decode_gencf((2, 8, 4, 8, 0 , 4 ,2))
    print_rel_error(drBApprox2, math.pi)
